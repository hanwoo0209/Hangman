...
class Hangman():
    ...
    def is_invalid_letter(self, input_):
        ...
        """
        return (not input_.isalpha()) or (len(input_)>1 and input_.isalpha())
    ...
    def print_game_status(self):
        ...
        print('\n')
        print('\n'.join(self.HANGMAN[:self.failed_attempts]))
        print('\n')
    ...
    def play(self):
        ...
        while True:
            ...
            if self.is_invalid_letter(user_input): # 플레이어가 입력한 인풋이 유효한 값인지 아닌지.
                print("제대로 입력하세") # 아니면, 제대로 입력하라는 메세지 출력하고
            ...
            if user_input in self.game_progress:
                print("이미 입력받은 알파벳입니다.") # 이미 입력받았던 알파벳이면, 이미 입력했다 메시지 출력
            ...
            if self.failed_attempts >= len(self.HANGMAN) :
              self.failed_attempts+=1
